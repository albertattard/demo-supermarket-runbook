...
- Add the `PICKUP` fulfillment type and the `PLACED` order status.
...
- Create immutable snapshots of product name, unit label, unit price, quantity, line total, submitted customer data, fulfillment type, goods subtotal, delivery fee, and grand total.
